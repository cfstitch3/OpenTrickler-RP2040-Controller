...
static void _servo_gate_set_current_state(float open_ratio) {
    ...
    float shutter0_range = servo_gate.eeprom_servo_gate_config.shutter0_close_duty_cycle -
                           servo_gate.eeprom_servo_gate_config.shutter0_open_duty_cycle;
    float shutter1_range = servo_gate.eeprom_servo_gate_config.shutter1_close_duty_cycle -
                           servo_gate.eeprom_servo_gate_config.shutter1_open_duty_cycle;

    shutter0_duty_cycle = _pwm_full_scale_level *
        (servo_gate.eeprom_servo_gate_config.shutter0_open_duty_cycle + shutter0_range * open_ratio);
    shutter1_duty_cycle = _pwm_full_scale_level *
        (servo_gate.eeprom_servo_gate_config.shutter1_open_duty_cycle + shutter1_range * open_ratio);
...
bool servo_gate_config_save(void) {
    bool is_ok = eeprom_write(
        EEPROM_SERVO_GATE_CONFIG_BASE_ADDR,
        (uint8_t *) &servo_gate.eeprom_servo_gate_config,
        sizeof(eeprom_servo_gate_config_t)
    );
    return is_ok;
}
